...
export default function Comfirmation() {
  ...
  const comfirm = order?.[order.length - 1];
  ...
  return (
    <div className="confirmation-container" key={comfirm.orderId}>
      ...
      <div className="confirmation-message">
        ...
        <h1>
          Your order <span>#{comfirm.orderId}</span> has been confirmed.
        </h1>

        <h1>
          A confirmation has been sent to{" "}
          <span>{comfirm?.customer?.email}</span>
        </h1>
      </div>
      ...
      <div className="confirmation-buttons">
        <NavLink className="active" to="/userDash">
          <button
            onClick={() => createNotification(comfirm.orderId, comfirm.status)}
          >
            Track Order
          </button>
        </NavLink>
